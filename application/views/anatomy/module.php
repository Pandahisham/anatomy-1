<div id="content">
	...
	<?php foreach ($modules as $m) { ?>
		<li><a href="<?= site_url('anatomy/module') . '/' . $m['module'] ?>">Module <?= $m['module'] ?></a></li>
	<?php } ?>
	</ul>
	<?php
	} else { 
	...
	<div id="quiz_info" class="round-big">
		...
		<a href="<?= site_url('anatomy/module') .'/'. $this->uri->segment(3) . '/new' ?>">Start Over</a><br />
		<a href="<?= site_url('anatomy/module') ?>">Change Module</a>
		</div>
	</div>
	
	<div class="filters">
		<form method="post" action="<?= site_url('anatomy/setFilters/module') ?>">
