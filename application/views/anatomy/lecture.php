<div id="content">
	...
	<?php foreach ($lectures as $l) { ?>
		<li><a href="<?= site_url('anatomy/lecture') . '/' . $l['id'] ?>"><?= $l['name'] ?></a></li>
	<?php } ?>
	</ul>
	<?php
	} else { 
	...
	<div id="quiz_info" class="round-big">
		...
		<a href="<?= site_url('anatomy/lecture') .'/'. $this->uri->segment(3) . '/new' ?>">Start Over</a><br />
		<a href="<?= site_url('anatomy/lecture') ?>">Change Lecture</a>
		</div>
	</div>
	
	<div class="filters">
		<form method="post" action="<?= site_url('anatomy/setFilters/lecture') ?>">
